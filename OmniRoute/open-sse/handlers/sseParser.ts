...
/**
 * Convert Perplexity Web2API SSE events into a single non-streaming response object.
 * Stores the final provider payload plus the reconstructed answer text for downstream translation.
 */
export function parseSSEToPerplexityWeb2ApiResponse(rawSSE, fallbackModel) {
  const lines = String(rawSSE || "").split("\n");
  const events = [];
  ...
  let accumulatedText = "";
  let selected = events[events.length - 1];

  for (const event of events) {
    accumulatedText = extractPerplexityWeb2ApiText(event, accumulatedText);
    if (isPerplexityWeb2ApiFinal(event)) {
      selected = event;
    }
  }
  ...
  return {
    ...selected,
    uuid: getPerplexityWeb2ApiMessageId(selected) || `pplx_${Date.now()}`,
    model: getPerplexityWeb2ApiModel(selected, fallbackModel || "default"),
