...
import {
  ...
  parseSSEToPerplexityWeb2ApiResponse,
...
export async function handleChatCore({
  ...
  // ChatGPT Web2API sometimes surfaces stale/rotated browser sessions as 429.
  // Retry once only for auth/session skew signals. Pure usage-limit 429s should
  // not trigger refresh-retry loops.
  if (provider === "chatgpt-web2api" && providerResponse.status === HTTP_STATUS.RATE_LIMITED) {
    const bodyPeek = await providerResponse
      .clone()
      .text()
      .catch(() => "");
    const looksLikeSessionSkew =
      /authentication token|session expired|session invalid|signing in again|sign in again|invalid session|could not parse your authentication token/i.test(
        bodyPeek
      );

    if (looksLikeSessionSkew) {
      const refreshed429 = (await refreshWithRetry(
        () => executor.refreshCredentials(credentials, log),
        1,
        log
      )) as null | {
      ...
      if (
        refreshed429?.accessToken ||
        refreshed429?.refreshToken ||
        refreshed429?.providerSpecificData
      ) {
        log?.info?.(
          "TOKEN",
          `${provider.toUpperCase()} | retry once after 429 with refreshed session`
        );
        Object.assign(credentials, refreshed429);

        if (onCredentialsRefreshed) {
          await onCredentialsRefreshed(refreshed429);
        }

        try {
          const retryModelId = String(translatedBody.model || currentModel || effectiveModel);
          const retry429Result = await executor.execute({
            model: retryModelId,
            body: translatedBody,
            stream: upstreamStream,
            credentials: getExecutionCredentials(),
          ...
          // Accept successful retry or any non-429 result as fresher truth.
          if (
            retry429Result.response.ok ||
            retry429Result.response.status !== HTTP_STATUS.RATE_LIMITED
          ) {
            providerResponse = retry429Result.response;
            providerUrl = retry429Result.url;
            providerHeaders = retry429Result.headers;
            finalBody = retry429Result.transformedBody;
            reqLogger.logTargetRequest(providerUrl, providerHeaders, finalBody);
          }
        } catch {
          log?.warn?.("TOKEN", `${provider.toUpperCase()} | retry after 429 refresh failed`);
        }
      }
    }
  }
