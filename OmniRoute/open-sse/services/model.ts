...
function buildAmbiguousModelResult(modelId, candidateProviders) {
  const aliasesForHint = candidateProviders.map((p) => PROVIDER_ID_TO_ALIAS[p] || p);
...
function resolveModelByProviderInference(modelId, extendedContext) {
  // Gemini family models could go to either gemini or gemini-web2api
  ...
  if (nonOpenAIProviders.length > 1) {
    return buildAmbiguousModelResult(modelId, nonOpenAIProviders);
  }
