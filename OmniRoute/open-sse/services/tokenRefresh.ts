...
/**
 * Refresh Perplexity Web2API cookies by calling /api/auth/session.
 *
 * Perplexity uses Cloudflare, so we pass the full stored cookie string
 * to the auth/session endpoint. Cloudflare bot-check cookies (cf_clearance,
 * __cf_bm, __cflb) included in the Set-Cookie response are merged back into
 * the stored cookie string so subsequent requests stay unblocked.
 *
 * The refreshed full cookie string is returned as `accessToken` so the
 * updateProviderCredentials path stores it back to DB automatically.
 */
export async function refreshPerplexityWebCookies(cookieString, log) {
  if (!cookieString || typeof cookieString !== "string") {
    log?.warn?.("TOKEN_REFRESH", "No cookie string available for Perplexity Web2API refresh");
    return null;
  }
  ...
  if (!validation.valid) {
    log?.warn?.(
      "TOKEN_REFRESH",
      `Perplexity Web2API session refresh failed (${validation.errorCode})`
    );
    return null;
  }

  const now = new Date().toISOString();
  log?.info?.("TOKEN_REFRESH", "Perplexity Web2API session validated");
...
/**
 * Refresh ChatGPT Web2API session by replaying the full browser cookie string
 * to /api/auth/session, then persist the refreshed access token + merged cookies.
 */
export async function refreshChatgptWebSession(
  cookieString,
  currentAccessToken,
  ...
  if (hasCookie) {
    ...
    if (validation.valid) {
      const now = new Date().toISOString();
      log?.info?.("TOKEN_REFRESH", "ChatGPT Web2API session validated");
      ...
      return {
        ...
        providerSpecificData: buildChatgptSessionProviderData(providerSpecificData || null, {
          ...
          accountId: validation.accountId || providerSpecificData?.workspaceId || null,
          planType: validation.planType || providerSpecificData?.workspacePlanType || null,
          validatedAt: now,
          refreshedAt: now,
          sessionSource: providerSpecificData?.sessionSource || "browser_cookie_header",
        }),
      };
    }
    ...
    if (!canFallbackToAccessToken) {
      log?.warn?.(
        "TOKEN_REFRESH",
        `ChatGPT Web2API session refresh failed (${validation.errorCode})`
      );
      return null;
    }
  ...
  const accessValidation = await validateChatgptAccessToken(currentAccessToken);
  if (!accessValidation.valid) {
    log?.warn?.(
      "TOKEN_REFRESH",
      `ChatGPT Web2API access token check failed (${accessValidation.errorCode})`
    );
    return null;
  }
  ...
  return {
    ...
    providerSpecificData: buildChatgptSessionProviderData(providerSpecificData || null, {
      ...
      validatedAt: now,
      refreshedAt: now,
      sessionSource: providerSpecificData?.sessionSource || "browser_session_payload",
    }),
  };
}
...
async function _getAccessTokenInternal(provider, credentials, log) {
  switch (provider) {
    ...
    // Perplexity Web2API: refresh by replaying the full cookie string to /api/auth/session
    // and merging fresh CF + session cookies from Set-Cookie response headers.
    case "perplexity-web2api":
      return await refreshPerplexityWebCookies(credentials.accessToken, log);

    // ChatGPT Web2API: refresh by replaying the full cookie string to /api/auth/session
    // and persisting the refreshed bearer access token and merged cookies.
    case "chatgpt-web2api":
      return await refreshChatgptWebSession(
        credentials.refreshToken,
...
export async function getAccessToken(provider, credentials, log) {
  ...
  const cacheKey = getRefreshCacheKey(provider, tokenForRefresh);
