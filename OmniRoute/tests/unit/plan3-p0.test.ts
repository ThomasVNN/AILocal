...
test("getModelInfoCore requires provider prefix for unprefixed Gemini family models", async () => {
  const info = await getModelInfoCore("gemini-2.5-pro", {});
  assert.equal(info.provider, null);
  assert.equal(info.errorType, "ambiguous_model");
  assert.ok(Array.isArray(info.candidateProviders));
...
test("getModelInfoCore keeps explicit Gemini provider prefixes working", async () => {
  const official = await getModelInfoCore("gemini/gemini-2.5-pro", {});
  const web2api = await getModelInfoCore("gemini-w2a/gemini-2.5-pro", {});

  assert.equal(official.provider, "gemini");
  assert.equal(official.model, "gemini-2.5-pro");
  assert.equal(web2api.provider, "gemini-web2api");
  assert.equal(web2api.model, "gemini-2.5-pro");
});
...
test("DefaultExecutor keeps official Gemini on x-goog-api-key auth", () => {
  ...
  assert.equal(headers["x-goog-api-key"], "AIzaOfficialGeminiKey123");
  assert.equal(headers.Authorization, undefined);
});
