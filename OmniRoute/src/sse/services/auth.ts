...
function getPerplexitySessionStatus(connection: ProviderConnectionView): string {
...
function isPerplexitySessionBlocked(connection: ProviderConnectionView): boolean {
  const status = getPerplexitySessionStatus(connection);
  return status === "reauth_required" || status === "session_expired" || status === "expired";
}

function getPerplexitySessionRank(connection: ProviderConnectionView): number {
  const status = getPerplexitySessionStatus(connection);
...
function getChatgptSessionStatus(connection: ProviderConnectionView): string {
  if (connection.providerSpecificData?.authMethod !== "web2api") return "";
  const status = connection.providerSpecificData?.sessionStatus;
  return typeof status === "string" ? status.trim().toLowerCase() : "";
}

function isChatgptSessionBlocked(connection: ProviderConnectionView): boolean {
  const status = getChatgptSessionStatus(connection);
  return status === "reauth_required" || status === "session_expired" || status === "expired";
}

function getChatgptSessionRank(connection: ProviderConnectionView): number {
  const status = getChatgptSessionStatus(connection);
  if (!status || status === "active") return 0;
  if (status === "session_stale" || status === "stale") return 1;
  return 2;
...
function getGeminiWebSessionStatus(connection: ProviderConnectionView): string {
  if (connection.providerSpecificData?.authMethod !== "web2api") return "";
  const authMode = normalizeStatus(toStringOrNull(connection.providerSpecificData?.authMode));
  const requestHeaders = asRecord(connection.providerSpecificData?.requestHeaders);
  const authorizationHeader = toStringOrNull(connection.providerSpecificData?.authorizationHeader);
  const cookieHeader =
    toStringOrNull(requestHeaders.Cookie) || toStringOrNull(requestHeaders.cookie) || null;
  const hasWebSessionMaterial = Boolean(authorizationHeader || cookieHeader);
  const accessTokenLooksLikeApiKey =
    /^AIza[0-9A-Za-z_-]{8,}$/i.test(connection.accessToken || "") ||
    /^secretref-[0-9A-Za-z_-]{4,}$/i.test(connection.accessToken || "");
  const apiKeyLooksLegacy =
    authMode === "x-goog-api-key" ||
    (!hasWebSessionMaterial &&
      (accessTokenLooksLikeApiKey ||
        /^AIza[0-9A-Za-z_-]{8,}$/i.test(connection.apiKey || "") ||
        /^secretref-[0-9A-Za-z_-]{4,}$/i.test(connection.apiKey || "")));

  if (apiKeyLooksLegacy) return "legacy_api_key";

  const status = connection.providerSpecificData?.sessionStatus;
  return typeof status === "string" ? status.trim().toLowerCase() : "";
}

function isGeminiWebSessionBlocked(connection: ProviderConnectionView): boolean {
  const status = getGeminiWebSessionStatus(connection);
  return (
    status === "reauth_required" ||
    status === "session_expired" ||
    status === "expired" ||
    status === "legacy_api_key"
  );
}

function getGeminiWebSessionRank(connection: ProviderConnectionView): number {
  const status = getGeminiWebSessionStatus(connection);
  if (!status || status === "active") return 0;
  if (status === "session_stale" || status === "stale") return 1;
  return 2;
...
>>>>>>> 08d0e9f8b4e412fea54cb5999c022bd368bfb9cd
    ...
    connections.forEach((c) => {
      ...
      } else if (provider === "perplexity-web2api" && isPerplexitySessionBlocked(c)) {
        log.debug(
          "AUTH",
          `  → ${c.id?.slice(0, 8)} | skipped sessionStatus=${getPerplexitySessionStatus(c)}`
        );
      } else if (provider === "chatgpt-web2api" && isChatgptSessionBlocked(c)) {
        log.debug(
          "AUTH",
          `  → ${c.id?.slice(0, 8)} | skipped sessionStatus=${getChatgptSessionStatus(c)}`
        );
      } else if (provider === "gemini-web2api" && isGeminiWebSessionBlocked(c)) {
        log.debug(
          "AUTH",
    ...
    if (provider === "perplexity-web2api") {
      orderedConnections = [...orderedConnections].sort((a, b) => {
        const sessionRank = getPerplexitySessionRank(a) - getPerplexitySessionRank(b);
        if (sessionRank !== 0) return sessionRank;
        return (a.priority || 999) - (b.priority || 999);
      });
    } else if (provider === "chatgpt-web2api") {
      orderedConnections = [...orderedConnections].sort((a, b) => {
        const sessionRank = getChatgptSessionRank(a) - getChatgptSessionRank(b);
        if (sessionRank !== 0) return sessionRank;
        return (a.priority || 999) - (b.priority || 999);
      });
