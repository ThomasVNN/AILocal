...
export function setQuotaCache(
  ...
  // Evict oldest entry when at the size limit (LRU: Map preserves insertion order)
  if (!cache.has(connectionId) && cache.size >= MAX_CACHE_ENTRIES) {
...
/**
 * Prune stale/expired entries from the in-memory cache.
 * Removes entries that have been exhausted beyond their TTL and will not be refreshed.
 * Called automatically from backgroundRefreshTick().
 */
export function pruneQuotaCache(): number {
  const now = Date.now();
  let removed = 0;
  for (const [key, entry] of cache.entries()) {
    const age = now - entry.fetchedAt;
    // Remove exhausted entries that are past their TTL and have no upcoming resetAt
    if (entry.exhausted && !entry.nextResetAt && age > EXHAUSTED_TTL_MS * 2) {
      cache.delete(key);
      removed++;
    }
    // Remove active entries that are very stale (missed many refresh cycles)
    if (!entry.exhausted && age > ACTIVE_TTL_MS * 3) {
      cache.delete(key);
      removed++;
    }
  }
  if (removed > 0) {
    console.debug(`[QuotaCache] Pruned ${removed} stale entries (cache size: ${cache.size})`);
  }
  return removed;
}
...
async function backgroundRefreshTick() {
  ...
  try {
    ...
    // Prune stale entries first to keep the cache bounded
    pruneQuotaCache();
