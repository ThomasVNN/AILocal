...
/**
 * Evict expired lockout entries from the in-memory cache.
 * Entries are safe to remove when lockedUntil is in the past and there are no recent attempts.
 * @returns {number} Number of entries removed
 */
export function evictExpiredLockouts(config = DEFAULT_CONFIG): number {
  const now = Date.now();
  const windowStart = now - config.attemptWindowMs;
  let removed = 0;
  for (const [id, state] of lockoutCache.entries()) {
    const isUnlocked = !state.lockedUntil || state.lockedUntil <= now;
    const hasRecentAttempts = state.attempts.some((t) => t > windowStart);
    if (isUnlocked && !hasRecentAttempts) {
      lockoutCache.delete(id);
      removed++;
    }
  }
  return removed;
