...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const discoveryConfidence = toNonEmptyString(modelsData.confidence);
    const discoverySource = toNonEmptyString(modelsData.source);
    const hasMeaningfulModelBreadth = Array.isArray(fetchedModels) && fetchedModels.length >= 6;
    const shouldSkipDestructiveSync =
      logProvider === "perplexity-web2api" &&
      (discoveryConfidence === "low" || discoverySource === "fallback_static") &&
      !hasMeaningfulModelBreadth;
    ...
    return NextResponse.json({
      ...
      ...(shouldSkipDestructiveSync
        ? {
            skippedSync: true,
            warning:
              "Skipped replacing models because Perplexity discovery confidence is low (fallback list).",
          }
        : {}),
