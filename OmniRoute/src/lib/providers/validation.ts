...
/** Default timeout for all provider validation fetches — prevents hanging requests */
...
export async function validateProviderApiKey({ provider, apiKey, providerSpecificData = {} }: any) {
  ...
  const SPECIALTY_VALIDATORS = {
    ...
    // Perplexity Web2API — validates full browser Cookie header string via auth/session endpoint
    "perplexity-web2api": async ({ apiKey }: any) => {
      const result = await validatePerplexitySessionCookie(apiKey, {
    ...
    // ChatGPT Web2API — validates full browser Cookie header string via auth/session endpoint
    "chatgpt-web2api": async ({ apiKey, providerSpecificData }: any) => {
      const sessionPayload =
        providerSpecificData?.sessionPayload ||
        providerSpecificData?.sessionJson ||
        providerSpecificData?.session ||
        null;
      ...
      const result = sessionPayload
        ...
        : await (async () => {
            const cookieString =
              providerSpecificData?.cookieString ||
              providerSpecificData?.refreshToken ||
              providerSpecificData?.sessionCookie ||
              (typeof apiKey === "string" && apiKey.includes("=") ? apiKey : "");
            const bearerTokenCandidate =
              providerSpecificData?.accessToken ||
              providerSpecificData?.oauthAccessToken ||
              providerSpecificData?.lastAccessToken ||
              (typeof apiKey === "string" && apiKey.includes(".") ? apiKey : "");

            return cookieString
              ? validateChatgptSessionCookie(cookieString, {
                  allowBareSessionToken: true,
                  signal: AbortSignal.timeout(VALIDATION_TIMEOUT_MS),
              ...
              : validateChatgptAccessToken(bearerTokenCandidate, {
                  signal: AbortSignal.timeout(VALIDATION_TIMEOUT_MS),
                });
          })();
    ...
    "gemini-web2api": async ({ apiKey, providerSpecificData }: any) => {
      const apiKeyText = typeof apiKey === "string" ? apiKey.trim() : "";
      const looksLikeHeaderDump =
        apiKeyText.includes("\n") ||
        /^cookie\s*:/i.test(apiKeyText) ||
        /authorization/i.test(apiKeyText) ||
        /(?:^|\s)(?:-b|--cookie)\b/i.test(apiKeyText) ||
        /\b(?:SAPISID|APISID|__Secure-1PAPISID|__Secure-3PAPISID)=/i.test(apiKeyText);
      const looksLikeAuthorizationHeader =
        /^bearer\s+.+$/i.test(apiKeyText) || /^sapisid(?:1p|3p)?hash\s+.+$/i.test(apiKeyText);

      const rawInput = looksLikeHeaderDump
        ? apiKeyText
        : {
            authorizationHeader:
              providerSpecificData?.authorizationHeader ||
