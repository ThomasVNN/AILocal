...
export function isCacheable(body, headers) {
  ...
  // Skip caching very large payloads to avoid expensive hashing on the hot path.
  // Default limit: 256 KB. Override via SEMANTIC_CACHE_MAX_PAYLOAD_BYTES env var.
  const maxPayloadBytes = parseInt(
    process.env.SEMANTIC_CACHE_MAX_PAYLOAD_BYTES || String(256 * 1024),
    10
  );
  try {
    ...
    // If we can't estimate size, allow caching to proceed
  }
