...
function looksLikeGeminiApiKey(value: unknown): boolean {
  const text = toNonEmptyString(value);
  return !!text && (/^AIza[0-9A-Za-z_-]{8,}$/i.test(text) || /^secretref-[0-9A-Za-z_-]{4,}$/i.test(text));
}

function parseProviderSpecificData(value: unknown): JsonRecord {
...
function normalizeLegacyGeminiWeb2ApiConnections(db: SqliteDatabase) {
  type GeminiLegacyRow = {
    id?: string;
    auth_type?: string;
  ...
  const rows = db
    .prepare(
      "SELECT id, auth_type, access_token, api_key, display_name, provider_specific_data FROM provider_connections WHERE provider = 'gemini-web2api'"
    )
    .all() as GeminiLegacyRow[];
  ...
  for (const row of rows) {
    const providerSpecificData = parseProviderSpecificData(row.provider_specific_data);
    const requestHeaders =
      providerSpecificData.requestHeaders &&
      typeof providerSpecificData.requestHeaders === "object" &&
      !Array.isArray(providerSpecificData.requestHeaders)
        ? (providerSpecificData.requestHeaders as JsonRecord)
        : {};

    const storedApiKeyCandidates = [
      decrypt(toNonEmptyString(row.api_key)) || null,
      decrypt(toNonEmptyString(row.access_token)) || null,
      toNonEmptyString(providerSpecificData.apiKey),
      toNonEmptyString(requestHeaders["x-goog-api-key"]),
      toNonEmptyString(requestHeaders["x-api-key"]),
    ];
    const storedApiKey =
      storedApiKeyCandidates.find((candidate) => looksLikeGeminiApiKey(candidate)) || null;
    const authMode = toNonEmptyString(providerSpecificData.authMode);

    if (authMode !== "x-goog-api-key" && !storedApiKey) {
      continue;
    }
    ...
    delete nextProviderSpecificData.authMethod;
    delete nextProviderSpecificData.authMode;
    delete nextProviderSpecificData.sessionStatus;
    delete nextProviderSpecificData.sessionSource;
    delete nextProviderSpecificData.authorizationHeader;
    delete nextProviderSpecificData.requestHeaders;
    delete nextProviderSpecificData.lastSessionError;
    delete nextProviderSpecificData.importedAt;
    delete nextProviderSpecificData.lastValidatedAt;
    delete nextProviderSpecificData.lastRefreshAt;
    delete nextProviderSpecificData.models;
    delete nextProviderSpecificData.apiKey;

    const currentDisplayName = toNonEmptyString(row.display_name);
    const nextDisplayName =
      currentDisplayName === "Gemini Web Session" ? "Gemini API Key" : currentDisplayName;

    update.run({
      id: row.id,
      provider: "gemini",
      authType: "apikey",
      apiKey: encrypt(storedApiKey),
      accessToken: null,
      refreshToken: null,
      expiresAt: null,
      tokenExpiresAt: null,
  ...
  if (migratedCount > 0) {
    console.log(
      `[DB] Migrated ${migratedCount} legacy Gemini Web2API API-key connection(s) to the official Gemini provider`
    );
  }
}
