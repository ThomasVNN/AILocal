...
export function sanitizeRequest(body, logger = console) {
  ...
  // Optimized scan: only join once, and only if we have content
  const fullText = contents.length === 1 ? contents[0] : contents.join("\n");

  // ── Prompt Injection Detection ──
  // Skip detection for very large bodies if configured or needed (latency vs security)
  ...
  if (injections.length > 0) {
    ...
    if (config.mode === "block" && highSeverity.length > 0) {
      if (logger.warn) {
        logger.warn(
          `[SANITIZER] Blocking request: ${injections.map((d) => d.pattern).join(", ")}`
        );
      }
      result.blocked = true;
      return result;
  ...
  if (config.piiRedaction) {
    const isRedact = config.mode === "redact";
    const piiResult = processPII(fullText, isRedact);
    result.piiDetections = piiResult.detections;

    if (piiResult.detections.length > 0) {
      if (isRedact) {
        // Only deep clone if we actually detected something to redact
