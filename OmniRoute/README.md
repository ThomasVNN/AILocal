...
→ Choose one:
   • Import Claude session (~/.claude/.credentials.json or raw access token)
   • OAuth login
→ Auto token refresh
...
**Import flow:** OmniRoute keeps imported Claude Code credentials under the existing `claude` provider, so OpenWebUI, OpenClaw, and any OpenAI-compatible client continue using the same OmniRoute API surface.

**Docker note:** The one-click local-file import reads `~/.claude/.credentials.json` from the OmniRoute host/container. If OmniRoute runs in Docker, mount `~/.claude` into the container or paste the JSON manually.
