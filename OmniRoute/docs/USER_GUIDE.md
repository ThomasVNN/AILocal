...
Imported Claude Code credentials are stored on the existing `claude` provider connection inside OmniRoute. Downstream apps keep calling OmniRoute only; they do not need a separate Claude Web2API service.

If OmniRoute runs in Docker, the **Import Local File** action only works when `~/.claude/.credentials.json` is mounted into the container. Otherwise paste the file contents manually.
